...
5. If `.claude/rules/personal-voice.md` exists and the draft is Personal-domain content, read it and use it to protect intentional Personal voice choices from over-smoothing.
...
Go through the rewritten draft sentence by sentence. Assign a score. Track which sentences fall below `min_score_threshold` from `config.yaml`.
...
- If the original sentence is slightly rough but clearly human and aligned with the Personal voice adapter, keep it instead of smoothing it for its own sake
