...
7. If `.claude/rules/personal-voice.md` exists and the draft is Personal-domain content, read it and give it precedence over generic rewriting instincts.
...
For Personal-domain content with a project voice adapter:
- Preserve direct judgment, specifics, parenthetical asides, mild uncertainty, and low-key humor when they improve the piece
- Do not smooth the draft into generic creator voice just because it sounds cleaner
...
2. For the rest, commit to the statement when the evidence is strong. If uncertainty is genuine, keep one direct hedge instead of hedging every clause.
