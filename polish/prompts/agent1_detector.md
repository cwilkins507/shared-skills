...
6. If `.claude/rules/personal-voice.md` exists and the draft is Personal-domain content, read it and treat its `allow` guidance as higher priority than borderline flagging rules in this prompt.
...
For Personal-domain content, do not flag these as problems by themselves:
- Parenthetical asides
- Mild uncertainty that stays under the configured hedge density
- One teasing line aimed at a bad move or decision
- Slightly uneven rhythm that still reads clearly
